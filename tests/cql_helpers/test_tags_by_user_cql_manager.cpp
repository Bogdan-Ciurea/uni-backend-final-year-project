/**
 * @file test_tags_by_user_cql_manager.cpp
 * @author Bogdan-Alexandru Ciurea (sc20bac@leeds.ac.uk)
 * @brief This file is responsible for testing the tags by user cql manager
 * @version 0.1
 * @date 2023-01-11
 *
...
class TestTagsByUserCqlManager : public ::testing::Test {
 public:
  TestTagsByUserCqlManager(){};

  virtual ~TestTagsByUserCqlManager(){};

  const std::string CASSANDRA_IP = "127.0.0.1";

  CqlClient* _cql_client = nullptr;
  TagsByUserCqlManager* _tags_by_user_cql_manager = nullptr;
  ...
  void connect() {
    ...
    _tags_by_user_cql_manager = new TagsByUserCqlManager(_cql_client);
  ...
  bool delete_questions_by_course() {
    CqlResult cql_result =
        _cql_client->execute_statement("TRUNCATE TABLE schools.tags_by_user;");

    return cql_result.code() == ResultCode::OK;
  }
};

TEST_F(TestTagsByUserCqlManager, WriteRelationship_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
...
TEST_F(TestTagsByUserCqlManager, WriteMultipleRelationships_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
...
TEST_F(TestTagsByUserCqlManager, ReadRelationship_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
  ...
  auto result = _tags_by_user_cql_manager->get_tags_by_user(1, temp_uuid);
...
TEST_F(TestTagsByUserCqlManager, DeleteRelationship_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
  ...
  auto [cql_read_result, questions] =
      _tags_by_user_cql_manager->get_tags_by_user(1, temp_uuid);
...
TEST_F(TestTagsByUserCqlManager, DeleteRelationshipsByTag_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
  ...
  auto result = _tags_by_user_cql_manager->get_tags_by_user(1, temp_uuid_2);
...
TEST_F(TestTagsByUserCqlManager, InsertRelationshipTwice_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
...
TEST_F(TestTagsByUserCqlManager, ReadNonexistentRelationship_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
...
TEST_F(TestTagsByUserCqlManager, DeleteNonexistentRelationship_Test) {
  if (_tags_by_user_cql_manager == nullptr) {
    // Test case is disabled
    return;
  }
