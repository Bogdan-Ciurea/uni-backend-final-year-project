/**
 * @file users_by_course_cql_manager.cpp
...
CqlResult UsersByCourseCqlManager::configure(bool init_db_schema) {
...
CqlResult UsersByCourseCqlManager::init_prepare_statements() {
  ...
  cql_result = _cql_client->prepare_statement(_prepared_get_users_by_course,
                                              GET_USERS_BY_COURSE);
  if (cql_result.code() != ResultCode::OK) {
    LOG_ERROR << "Failed to initialise select users by course prepared "
                 "statement: "
              << cql_result.str_code() << cql_result.error();
  ...
  if (cql_result.code() != ResultCode::OK) {
    LOG_ERROR << "Failed to initialise delete all relationship of a course "
                 "prepared statement: "
              << cql_result.str_code() << cql_result.error();
  }
...
CqlResult UsersByCourseCqlManager::init_schema() {
  ...
  cql_result = _cql_client->execute_statement(CREATE_USERS_BY_COURSE_TABLE);

  return cql_result;
}

CqlResult UsersByCourseCqlManager::create_relationship(
    const int school_id, const CassUuid& course_id, const CassUuid& user_id) {
...
UsersByCourseCqlManager::get_users_by_course(const int school_id,
  ...
  CassStatementPtr statement(
      cass_prepared_bind(_prepared_get_users_by_course.get()));
  cass_statement_bind_int32(statement.get(), 0, school_id);
  cass_statement_bind_uuid(statement.get(), 1, course_id);
...
CqlResult UsersByCourseCqlManager::delete_relationship(
    const int school_id, const CassUuid& course_id, const CassUuid& user_id) {
  std::string error_message;
...
CqlResult UsersByCourseCqlManager::delete_relationships_by_course(
